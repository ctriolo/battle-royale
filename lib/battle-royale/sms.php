...
function send_sms(/*$to, $msg*/) {
  ...
  if ($GLOBALS['DEBUG']) { print('TO   '.$to.': '.$msg.PHP_EOL); return; }
...
function process_join($from, $body) {
  ...
  // error: no game name
  if (count($tokens) < 2) {
    send_reply(SMS_RESPONSE_JOIN_ERROR_NO_CODE);
    return;
  }
...
function process_confirm($from, $body) {
  ...
  if ($body == "y") {
	...
	send_sms($participant['phone'],
                 SMS_RESPONSE_CONFIRM_ANNOUNCE_WINNER,
                 $killer['name']);
      }
    } else {
...
function process_sms($from, $body) {
  if ($GLOBALS['DEBUG']) print('FROM '.$from.': '.$body.PHP_EOL);
  $_REQUEST['From'] = $from; // hack to get testing working, TODO: remove
  $body = trim($body);
  $person = find_person(array('phone' => $from));
  ...
  case SMS_STATE_CREATE: process_create($from, $body); return;
  default: break;
  }
  ...
  case SMS_KEYWORD_CREATE: process_create($from, $body); return;
  case SMS_KEYWORD_JOIN: process_join($from, $body); return;
  case SMS_KEYWORD_BEGIN: process_begin($from, $body); return;
  case SMS_KEYWORD_KILL: process_kill($from, $body); return;
  case SMS_KEYWORD_HELP: send_reply(SMS_RESPONSE_HELP); return;
  default:   send_reply(SMS_RESPONSE_DERP); return;
  }
